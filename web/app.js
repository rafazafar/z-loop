...
function renderCurrentWork() {
  ...
  updateHtml($("#currentWork"), `
    <header class="work-head">
      <div><h3><a href="${issueUrl}" target="_blank" rel="noopener noreferrer">Issue #${ticket.id}</a> · ${esc(ticket.title || "Tracked work")}</h3><p>${ticket.pr ? `<a href="${esc(ticket.pr)}" target="_blank" rel="noopener noreferrer">PR ${ticket.prNumber}</a> · ${esc(ticket.prState || "unknown")}${ticket.conflicted ? ' <span class="badge-conflict">CONFLICTS WITH MAIN</span>' : ''}` : "No PR"}${ticket.headShort ? ` · head ${esc(ticket.headShort)}` : ""} · revision ${ticket.round} · P0/P1 repair ${ticket.repair}/${ticket.repairLimit}${ticket.session ? ` · session ${workerDuration(ticket)}` : ""}</p></div>
...
function renderTickets() {
  updateHtml($("#tickets"), activeTickets().map((ticket) => {
    ...
    return `<article class="ticket-row">
      ...
      <div class="ticket-pr">${ticket.pr ? `<a href="${esc(ticket.pr)}" target="_blank" rel="noopener noreferrer">PR ${ticket.prNumber}</a> · ${esc(ticket.prState || "unknown")}${ticket.conflicted ? ' <span class="badge-conflict">CONFLICTS</span>' : ''}` : "No PR"}<span>${ticket.headShort ? `head ${esc(ticket.headShort)} · ` : ""}last activity ${formatAge(ticket.lastActivity)}</span></div>
